...
printk("estack:%p,oist:%p,%d,%lx\n",estacks,oist,sizeof(exception_stack_sizes),sizeof(exception_stacks));
    ...
    for (int v = 1; v < N_EXCEPTION_STACKS + 1; v++) {
        ...
        if (v == DEBUG_STACK) {
            ...
            void * pstack = alloc_vm_stack(PAGE_SIZE);
            char *pend = pstack = (char *)pstack + PAGE_SIZE - 64;
    ...
    for (int v = 1; v < N_EXCEPTION_STACKS + 1; v++) {
        void * pstack = alloc_vm_stack(PAGE_SIZE);
        char *pend = (char *)pstack + PAGE_SIZE - sizeof(long);
        printk("ist:%d,from:%016lx to:%016lx\n", v, pstack, pend);
        register_stack((ulong)pstack, (ulong)pend, istname[v],p->cpu);
        oist->ist[v] = p->arch_cpu.atss.tss.ist[v] = (unsigned long)pend;

        if (v == PG_STACK) {
            map_alloc_at((ulong)pstack +PAGE_SIZE - exception_stack_sizes[v], exception_stack_sizes[v]-PAGE_4K_SIZE, map_flags_stack());
            pend = (char *)pstack + PAGE_SIZE - 64;
