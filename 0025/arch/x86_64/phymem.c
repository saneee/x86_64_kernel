...
static ulong smallbits[PAGE_SIZE/PAGE_4K_SIZE/sizeof(long)];
...
ulong alloc_small_phy_page_safe()
{
    ...
    if (!smallpages) {
        ...
        smallpnow = smallbits;
        smallpmax = smallbits + (PAGE_SIZE / PAGE_4K_SIZE) /64;
        smallnowbit = 0;
        smallpages = PAGE_SIZE / PAGE_4K_SIZE;
