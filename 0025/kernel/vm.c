...
{
    unmap_free_at(addr, size);
    free_linear(VM_ZONE_KERNEL_STACK, addr - PAGE_SIZE, size + PAGE_SIZE);
}
...
void *alloc_vm_stack(ulong size)
{
    ...
    void *p = alloc_linear(VM_ZONE_KERNEL_STACK, size + PAGE_SIZE);	//one more page

    if (!p)
        return p;
    addr = (ulong) p + PAGE_SIZE;	//the lowest page is reserved for stack overflow check
    if (OK!=map_alloc_at(addr+size-PAGE_4K_SIZE,PAGE_4K_SIZE,map_flags_stack())) {
        free_linear(VM_ZONE_KERNEL_STACK,addr-PAGE_SIZE,size+PAGE_SIZE);
        return 0;
    }
    return (void *)(addr);
