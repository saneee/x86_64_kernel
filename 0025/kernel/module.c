...
__init static int init_module_ap()
{
    ...
    int *pstatus = this_cpu_ptr(&percpu_status[0]);
...
static __init int init_module_call(bool isbp)
{
    if (isbp) {
        init_module();
    }
...
static __init int init_module_step(bool isbp, int step )
{
    int *pstatus = this_cpu_ptr(&percpu_status[0]);
printk("isbp:%d,step:%d\n",isbp,step);
    for (int i = 0; i < total; i++) {
printk("pstatus:%016lx,name:%s,status:%d\n",pstatus,all[i].name,pstatus[i]);
        if (all[i].status == MODULE_LOADED) {
            (all[i].handler) (&all[i], step, all[i].arg);
...
static __init int init_module_device(bool isbp)
{
    return init_module_step(isbp,isbp?MOD_BPDEVICE:MOD_APDEVICE);
}
static __init int init_module_subsys(bool isbp)
{
    return init_module_step(isbp,isbp?MOD_BPSUBSYS:MOD_APSUBSYS);
}

pure_initcall(init_module_call);
device_initcall(init_module_device);
subsys_initcall(init_module_subsys);
