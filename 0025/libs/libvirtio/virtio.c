...
void virtio_get_buf_finalize(virtio_dev_t dev, int qidx) {
    ...
    q = &dev->queues[qidx];
    vring = &q->vring;
    printk("avail flag:%x,used flag:%x,%x，last_used:%x ",vring->avail->flags,vring->used->flags,*q->used_event,q->last_used);
