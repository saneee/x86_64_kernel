...
printk("pdpte:%lx,%d\n",first_pdp.pdpte[idx],idx);
    ...
    for (int i = 0; i < PD_PER_PTE; i++) {
        ...
        phy_first_pd.pde[i] = i * PAGE_SIZE | PTE_PS | PTE_W | PTE_P;
        phy_second_pd.pde[i] =
            (i + PD_PER_PTE) * PAGE_SIZE | PTE_PS | PTE_W | PTE_P;
        phy_third_pd.pde[i] =
            (i + 2 * PD_PER_PTE) * PAGE_SIZE | PTE_PS | PTE_W | PTE_P;
...
printk("max_addr:%lx\n",max_addr);
        for (; addr + PAGE_SIZE < max_addr; addr += PAGE_SIZE) {
...
*/
        ...
        for (; addr  < __max_phy_mem_addr; addr += PAGE_SIZE) {
            if (OK != map_page_p2v(addr, P2V(addr), PTE_PS | PTE_P | PTE_W)) {
                panic("Not enough init heap memory!");
            }
        }
...
void unmap_free_at(ulong vaddr, size_t size)
{
    ...
    if ((*pteaddr & (PTE_PS|PTE_P))==(PTE_PS|PTE_P)) {
        ...
        //2M page
       if (align2Mvaddr == vaddr && PAGE_SIZE<=size) {
