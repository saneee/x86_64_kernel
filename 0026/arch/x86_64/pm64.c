...
DEFINE_PER_CPU(char *, pg_stack_ptr);

DEFINE_PER_CPU(int, irq_count) __visible = -1;
DEFINE_PER_CPU(int, tasklet_count) __visible = -1;
...
char *cpu_base[MAX_CPUS]={[0]=init_percpu_start};
extern int nr_cpu;
void test()
{
    print_regs();
...
static char *istname []={"","ist1","ist2","ist3","ist4","ist5","ist6","ist7"};
...
printk("estack:%p,oist:%p,%d,%lx\n",estacks,oist,sizeof(exception_stack_sizes),sizeof(exception_stacks));
    ...
    for (int v = 1; v < N_EXCEPTION_STACKS + 1; v++) {
        ...
        printk("ist:%d,from:%016lx to:%016lx\n", v, estacks - exception_stack_sizes[v], estacks);
        register_stack((ulong)estacks - exception_stack_sizes[v], (ulong)estacks, istname[v]);
        ...
        if (v == DEBUG_STACK) {
            ...
            void * pstack = alloc_vm_stack(PAGE_SIZE_LARGE);
            char *pend = pstack = (char *)pstack + PAGE_SIZE_LARGE - 64;
            PER_CPU_PTR_VAR(&irq_stack_ptr,p) = (char *)pend;
            printk("irq_stack_ptr:%016lx\n",pend);
            register_stack((ulong)pstack, (ulong)pend, "irq");
    ...
    for (int v = 1; v < N_EXCEPTION_STACKS + 1; v++) {
        void * pstack = alloc_vm_stack(PAGE_SIZE_LARGE);
        char *pend = (char *)pstack + PAGE_SIZE_LARGE - sizeof(long);
        printk("ist:%d,from:%016lx to:%016lx\n", v, pstack, pend);
        register_stack((ulong)pstack, (ulong)pend, istname[v],p->cpu);
        oist->ist[v] = p->arch_cpu.atss.tss.ist[v] = (unsigned long)pend;

        if (v == PG_STACK) {
            map_alloc_at((ulong)pstack +PAGE_SIZE_LARGE - exception_stack_sizes[v], exception_stack_sizes[v]-PAGE_4K_SIZE, map_flags_stack());
            pend = (char *)pstack + PAGE_SIZE_LARGE - 64;
            PER_CPU_PTR_VAR(&pg_stack_ptr,p) = (char *)pend;

        } 
        if (v == DEBUG_STACK) {
            PER_CPU_PTR_VAR(&debug_stack_addr,p) = (unsigned long)pend;
        }
        else if (v == IRQ_STACK) {
            //top 64 used in ist:ss,rsp,flag,cs,ip,errno etc
            pend = (char *)pstack + PAGE_SIZE_LARGE - 64;
            PER_CPU_PTR_VAR(&irq_stack_ptr,p) = (char *)pend;
            printk("irq_stack_ptr:%016lx\n",pend);
            register_stack((ulong)pstack, (ulong)pend, "irq", p->cpu);
...
static inline void init_percpu(cpu_p p)
{
    ...
    PER_CPU_PTR_VAR(&irq_count,p) = -1;
    PER_CPU_PTR_VAR(&except_count,p) = -1;
    PER_CPU_PTR_VAR(&tasklet_count,p) = -1;
    PER_CPU_PTR_VAR(&this_cpu_off,p) = (unsigned long)p->percpu_base;
    PER_CPU_PTR_VAR(&cpu_number,p) = p->cpu;
    irq_cpustat_t *pstat = PER_CPU_PTR(&irq_stat, p);
    printk("cpu:%d irq_stat:%lx ,cpu:%lx,base:%lx\n",p->cpu,pstat,p,p->percpu_base);
...
void init_idt()
{
    ...
    set_intr_gate(X86_TRAP_PF, PG_STACK);
    desc.limit = sizeof(_idt) - 1;
    desc.addr = (ulong) & _idt;
    lidt(&desc);
...
void bootup_aps(void)
{
    ...
    for (int i = 0; i < nr_cpu; i++) {
        ...
        *(uint32 *) (code - 8) = K2P((u64) entry32mp);
