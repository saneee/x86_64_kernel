...
{
    ret_t (*pfunc)(vm_zone_t id,size_t size)=yaoscall_p(YAOS_alloc_linear);
    ret_t ret=(*pfunc)(id, size);
...
static inline void * yaos_alloc_linear_low_mmap(size_t size)
{
    return yaos_alloc_linear(VM_ZONE_LOW_MMAP,size);
}
