...
extern void entrycall(u32 addr,u32 magic,u32 entry);
u32 bootmain()
{
  struct mbheader *hdr;
  void (*entry)(u32,u32);
  ...
  x = (uint32*) 0x100000; // scratch space
...
found_it:
  ...
  // Does not return!
  //entry = (void(*)(unsigned int addr,unsigned int magic))(hdr->entry_addr);
  //entrycall(addr,magic,hdr->entry_addr);
