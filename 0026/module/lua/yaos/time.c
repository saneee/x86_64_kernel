...
//already in poll cb now.
static void timeout_cb (u64 nownsec,void *p)
{
    struct lua_poll *p_poll = (struct lua_poll *)p;
    p_poll->data = (void *)(nownsec / 1000000000UL);
    p_poll->ret = OK;
    lua_State * L = p_poll->L;
    ASSERT(p_poll->callback != -1);
    lua_rawgeti(L, LUA_REGISTRYINDEX, p_poll->callback);
    if (lua_isfunction(L, -1)) {
       lua_pushinteger(L,(ulong)p_poll->data);
       lua_call(L,1,0);
    } else {
        //lua_pushnumber(L,(ulong)p->data);
        //yaos_lua_promise_fire(L);
        printk("type:%d,callback%d\n",lua_type(L, -1),p_poll->callback);
        ASSERT(0);//now use callback all
    }
    luaL_unref(L, LUA_REGISTRYINDEX, p_poll->callback);
    p_poll->callback = -1;
    yaos_mfree(p);
...
static void yaos_lua_create_timer(struct lua_poll *p_poll)
{
    ...
    set_timeout_nsec_poll(timeout*1000000UL,timeout_cb,p_poll);
