...
// Used for parsing MSI-x
    ...
    printk("msix_table_bar:%d,val:%x,off:%x\n", pci->_msix.msix_table_bar, val, off + PCIR_MSIX_TABLE);
...
//printk("pci_get_bar:%d,%d,%lx\n",n-1,pci->_bars[n - 1]._pos,pci->_bars[n - 1]._addr_64);
...
//printk("bar->dev:%lx,addr:%lx,pos:%d\n",msix_bar->_dev,msix_bar->_addr_64,msix_bar->_pos);
...
bool pci_msi_write_entry(pci_device_t pci, int entry_id, struct msi_msg *msg)
{
    if (!pci_is_msi(pci)) {
        return false;
    }
...
void pci_msi_enable(pci_device_t pci)
{
    if (!pci_is_msi(pci)) {
        return;
    }
...
void pci_msi_disable(pci_device_t pci)
{
    if (!pci_is_msi(pci)) {
        return;
    }
...
printk("msix_mask_entry:%d,%lx,%x\n",entry_id,ctrl,ctrl_data);
    return true;
}
...
bool msix_unmask_entry(pci_device_t pci, int entry_id)
{
    ...
    u32 ctrl_data = mmio_getl(ctrl);
printk("msix unmask entry :%016lx,%x\n",ctrl,ctrl_data);
    ctrl_data &= ~(1 << MSIX_ENTRY_CONTROL_MASK_BIT);
    mmio_setl(ctrl, ctrl_data);
printk("unmask :%x,%x,%x\n",ctrl_data,mmio_getl(ctrl),ctrl_data);
    return true;
...
void msix_mask_all(pci_device_t pci)
{
    if (!pci_is_msix(pci)) {
        return;
    }
...
void pci_msix_enable(pci_device_t pci)
{
    ...
    printk("pci_msix_enable msix_bar:%lx,%d\n", msix_bar, pci->_msix.msix_table_bar);
    if (!msix_bar)
        return;
...
bool pci_msix_write_entry(pci_device_t pci, int entry, struct msi_msg *msg)
{
    u64 addr = ((u64)msg->address_hi<<32) + (u64)msg->address_lo;
    u32 data = msg->data;
...
printk("msix_write_entry addr:%lx,msiaddr:%lx,data:%x\n",entryaddr,addr,data);
    mmio_setl(entryaddr + (u8) MSIX_ENTRY_ADDR_LO, msg->address_lo);
    mmio_setl(entryaddr + (u8) MSIX_ENTRY_ADDR_HI, msg->address_hi);

  
//    mmio_setq(entryaddr + (u8) MSIX_ENTRY_ADDR, addr);
    mmio_setl(entryaddr + (u8) MSIX_ENTRY_DATA, data);
printk("get:%lx,%x\n",mmio_getq(entryaddr + (u8) MSIX_ENTRY_ADDR),mmio_getq(entryaddr + (u8) MSIX_ENTRY_DATA));
    return true;
}
