...
static void msec_callback(u64 nsec)
{
   extern u64 msec_count;
   void run_local_timers(void);
   msec_count = hpet_uptime_msec();
   //msec_count++
   set_timeout_nsec(1000000, msec_callback);
   run_local_timers();
}
static void msec_apcallback(u64 nsec)
{
   extern u64 msec_count;
   void run_local_timers(void);
   set_timeout_nsec(1000000, msec_apcallback);
   run_local_timers();
}

static __init int init_irq_call(bool isbp)
{
    if(isbp)set_timeout_nsec(1000000, msec_callback);
    else set_timeout_nsec(1000000, msec_apcallback);
    sti();
    return 0;
}
