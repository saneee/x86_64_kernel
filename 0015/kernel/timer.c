...
static DEFINE_PER_CPU(struct timer_manager,mtime);
...
int set_timeout_nsec(u64 nsec, void (*pfunc) (u64 nownsec))
{
    __thread struct timer_manager *ptime = PERCPU_PTR(&ntime);
    struct timeout_callback *p = ptime->pfree;
...
{
    struct timer_manager *ptime = PERCPU_PTR(&ntime);
