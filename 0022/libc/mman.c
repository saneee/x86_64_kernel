...
int munmap(void *addr, size_t length)
{
    ...
    ret_t ret = yaos_free_at((ulong)addr,length);
...
void *__mremap(void *old_addr, size_t old_len, size_t new_len, int flags, ...)
{
    ...
    if (!(flags & MREMAP_FIXED) || !new_addr) {
        new_addr = yaos_alloc_linear(new_len);
    ...
    if (new_len >old_len) {
        ...
        yaos_free_at((ulong)new_addr+new_len, old_len - new_len);
    }
    if (yaos_map_copy((ulong)new_addr, (ulong)old_addr, old_len) != OK) return MAP_FAILED;
    if (!(flags & MREMAP_DONTUNMAP)) {
        yaos_free_at((ulong)old_addr, old_len);
    }
    return new_addr;
}
