...
void virtio_setup_features(virtio_dev_t p)
{
    ...
    u64 drv_features = virtio_get_driver_features();
...
static void init_phys_queue(struct virtio_queue *q)
{
    ...
    q->last_used_idx=0;
    printk("num:%d used_event:%016lx, avail_event:%016lx,avail:%016lx,user:%016lx\n",q->vring.num,q->used_event,q->avail_event,q->vring.avail->ring,q->vring.used->ring);
    return;
}
