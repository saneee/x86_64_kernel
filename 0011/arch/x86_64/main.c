...
void bp_main(u32 info_addr, u32 magic)
{
    ...
    uart_early_init();
    init_cpuid();
    printk("before alternative_instructions memcpy:\n");
    dump_mem(__memcpy,0x20);
   
    alternative_instructions();
    printk("after alternative_instructions memcpy:\n");
    dump_mem(__memcpy,0x20);
    
    __memcpy(buf,__memcpy,0x20);
    printk("__memcpy(buf,__memcpy,0x20);\n");
    dump_mem(buf,0x20);
    for(;;); 
