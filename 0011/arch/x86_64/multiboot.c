...
void add_reserve(u64 addr, u64 size)
{
    ...
    if (P2V(addr) + size > max_reserve_addr)
    ...
    if (low4k) {
        ...
        if (low4ksize >= size) {
            if (addr + size < __max_phy_mem_addr) {
                free_kheap_small(P2V(addr), size);
            }
        }
        size -= low4ksize;
        free_kheap_small(P2V(addr), low4ksize);
        addr += low4ksize;
        on_mem(addr, size);
        return;
    }
    ...
    if (size < PAGE_SIZE) {
        ...
        if (low) {
            ...
            if (addr + lowsize < __max_phy_mem_addr) {
                free_kheap_4k(P2V(addr), lowsize);
            }
            addr += lowsize;
            on_mem(addr, size);
            return;
...
void init_e820()
{
    ...
    add_reserve(V2P(kernel_start), kernel_end - kernel_start);
...
print_regs();
    ...
    if (CHECK_FLAG(mbi->flags, 6)) {
        ...
        e820map_addr = (ulong) P2V(mbi->mmap_addr);
