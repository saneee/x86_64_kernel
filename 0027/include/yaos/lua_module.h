...
#define DO_LUA_FILE(L,name) \
  LOAD_LUA_FILE(L,name)\
  lua_pcall(L, 0, LUA_MULTRET, 0)
typedef struct lua_file lua_file_t;
struct lua_file {
    const char * buf;
    size_t size;
    const char *name;
...
#define DEFINE_LUA_FILE(name) \
    DECLARE_LUA_FILE(name);\
    static struct lua_file _lua_file_##name   \
        __attribute__((__used__))\
        __attribute__((section(".lua_file_data")))={\
      LUA_FILE_START(name),(size_t)LUA_FILE_SIZE(name),#name}
...
DECLARE_PER_CPU(lua_State *, vmL);
static inline lua_State * getL()
{
    return this_cpu_read(vmL);
}
int do_lua_file(lua_State *L, char *name);
int load_lua_file(lua_State *L, char *name);

extern lua_file_t _lua_file_data_start[];   //kernel64.ld
extern lua_file_t _lua_file_data_end[];
