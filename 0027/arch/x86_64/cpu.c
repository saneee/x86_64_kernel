...
ulong get_call_stack(int lvl)
{
    ulong *p = (ulong *)read_rbp();
...
void show_call_stack(int lvl)
{
    ...
    while(--lvl) {
        ...
        p = (ulong *)*p;
        if (!(((ulong)p>KERNEL_STACK_ZONE_START && (ulong)p<KERNEL_STACK_ZONE_END)
           || ((ulong)p>KERNEL_BASE) || get_phy_addr((ulong)p))) break;

        sprintf(&buf[pos],"%016lx ",*(p+1));
        pos += 17; 
...
void cprint_regs(struct trapframe *tf)
{
    ...
    spin_lock(&spin_reg);
    __show_regs(tf);
    ulong start = tf->rsp-128;
    ulong end = (start + PAGE_4K_SIZE -1)&~0xfff;
    printk("dump_mem64:%lx,%lx\n",start,end-start-32);
    dump_mem64((char *)start,end-start-32);
    ulong *p = (ulong *)tf->rbp;
    printk("call stack:\n");
    while(((ulong)p>KERNEL_STACK_ZONE_START && (ulong)p<KERNEL_STACK_ZONE_END) 
           || ((ulong)p>KERNEL_BASE) || get_phy_addr((ulong)p)){
        printk(" %016lx ",*(p+1));
        if (p == (ulong *)*p)break;
        if (*(p+1)<KERNEL_BASE) break;
        p = (ulong *)*p;
    }
    printk("\n");
