...
{
    if (!size) return OK;
    ulong vend = oldaddr + size;
    while(oldaddr < vend) {
        ret_t af = map_paddr_flags_at(oldaddr);
        if (!af.v) return EINVAL;
        ulong paddr = af.v;
        ulong flag = af.e;
        if(map_4k_page_p2v(paddr,vaddr,flag) != OK) return ENOMEM;
        vaddr += PAGE_4K_SIZE;
        oldaddr += PAGE_4K_SIZE;
    }
    return OK;
}
...
void unmap_free_at_impl(ulong vaddr, size_t size, bool isfree)
{
    size_t len = 0;
    if (size ==0 ) return;
    printk("unmap_free_at:%lx,size:%lx,isfree:%d\n",vaddr,size,isfree);
    ulong paddr;
    ulong align2Mvaddr = vaddr & ~PAGE_2M_MASK;
    ...
    if ((*pteaddr & (PTE_PS|PTE_P))==(PTE_PS|PTE_P)) {
       ...
       if (align2Mvaddr == vaddr && PAGE_SIZE_LARGE<=size) {
           ...
           if(isfree) free_phy_one_page(*pteaddr & ~PAGE_2M_MASK);
           *pteaddr = 0;
           flush_tlb_one_page(vaddr);

           unmap_free_at_impl(vaddr+PAGE_SIZE_LARGE,size-PAGE_SIZE_LARGE, isfree);
           return;   
       } else {
             //remap 2M to 4K page
    ...
    while (len<size) {
        ...
        if( isfree) free_phy_page_small(paddr, PAGE_4K_SIZE);
    ...
    if (len<size) unmap_free_at_impl(vaddr+len,size-len, isfree);
    return; 
