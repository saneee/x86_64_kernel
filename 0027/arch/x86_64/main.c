...
void bp_main(u32 info_addr, u32 magic)
{
    ...
    uart_early_init();
#if 0
    extern struct poll_cb clean_poll;
    struct poll_cb *p = (struct poll_cb *)((ulong)init_percpu_start+(ulong)&clean_poll);
    printk("p:%lx,p->handle:%lx,p->next:%lx\n",p,p->handle,p->pnext);
    dump_mem((char*)0xffffffff81011910,100);
    void set_data_break(ulong addr);
    set_data_break(0xffffffff8101194ful);
#endif
    init_cpuid();
