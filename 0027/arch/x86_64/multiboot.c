...
static ulong kernel_start;
static ulong kernel_end;
void init_bss()
{
    ...
    kernel_end = __kernel_end+init_heap_size;
    kernel_start = __kernel_start;
printk("kstart:%lx,kend:%lx,pcpu:%lx,pcpuend:%lx\n",kernel_start,kernel_end,init_percpu_start,init_percpu_end);
}
...
static void remap_iomem(void)
{
    for (int i = 0; i < remaped; i++) {
	    ...
	    printk("pg:%d,%lx,%lx\n",pg,pg*PAGE_SIZE_LARGE,pg*PAGE_SIZE_LARGE+IO_MEM_BASE);
            //do not cache io memory 
            if (OK !=
                map_page_p2v(pg * PAGE_SIZE_LARGE, pg * PAGE_SIZE_LARGE + IO_MEM_BASE,
...
{
    ulong low = addr & (PAGE_SIZE_LARGE - 1);

    if (!addr)
        return;
    if (low) {
        ulong lowsize = PAGE_SIZE_LARGE - low;
        long pg;

        if (lowsize >= size) {
            pg = (addr + PAGE_SIZE_LARGE - 1) / PAGE_SIZE_LARGE;
            add_map(pg);
            return;
        }
        size -= lowsize;
    ...
    while (size >= PAGE_SIZE_LARGE) {
        add_map(addr / PAGE_SIZE_LARGE + 1);
...
void add_reserve(u64 addr, u64 size)
{
    ...
    if (P2K(addr) + size > max_reserve_addr)
        max_reserve_addr = addr + size;
    printk("add reserve:%lx %lx\n",addr,size);
}
...
{
    printk("deal_reserve %lx,%lx,max:%lx\n",addr,size,max_reserve_addr);
    ...
    for (int i = 0; i < reserved_nr; i++) {
        ...
        if (addr >= rev_start && addr + size <= rev_start + rev_size)
            ...
            newsize = size + addr - newaddr;
            DEBUG_PRINT("newaddr:%lx,newsize:%lx,rev_start:%lx,rev_size:%lx\n",newaddr,newsize,rev_start,rev_size);
            //chunk in reserve
            ...
            DEBUG_PRINT("newaddr:%lx,newsize:%lx,rev_start:%lx,rev_size:%lx\n",addr,newsize,rev_start,rev_size);

            deal_reserve(addr, newsize, ismem);
            ulong newaddr = rev_start + rev_size;
            if (size + addr > newaddr) {
                newsize = size + addr - newaddr;
                DEBUG_PRINT("newaddr:%lx,newsize:%lx,rev_start:%lx,rev_size:%lx\n",newaddr,newsize,rev_start,rev_size);
...
static void on_mem(u64 addr, u64 size)
{
    ...
    if (low4k) {
        ...
        if (low4ksize >= size) {
            if (addr + size < __max_phy_mem_addr) {
                free_kheap_small(P2K(addr), size);
            }
        }
        size -= low4ksize;
        free_kheap_small(P2K(addr), low4ksize);
        addr += low4ksize;
        on_mem(addr, size);
        return;
    }
    ...
    if (size < PAGE_SIZE_LARGE) {
        ...
        npg = size / PAGE_SIZE_LARGE;
        free_phy_pages(addr, npg * PAGE_SIZE_LARGE);
        size &= PAGE_SIZE_LARGE - 1;
        addr += npg * PAGE_SIZE_LARGE;
        if (size == 0)
            return;
        if (addr + size < __max_phy_mem_addr) {
            free_phy_page_small(addr, size);
...
void init_e820()
{
    ...
    add_reserve(K2P(kernel_start), kernel_end - kernel_start);
    add_map(IOAPIC_BASE/PAGE_SIZE_LARGE+1);//map ioapic
...
void init_page_map(void)
{
    ...
    //add_reserve(0, reserve_low_size);
    //add_reserve(V2K(kernel_start),kernel_end - kernel_start);
    ...
    for (mmap = (memory_map_t *) (pe820);
        ...
        msize <<= 32;
        msize += mmap->length_low;
        printk("addr:%lx,size:%lx,lastend:%lx,type:%d\n",addr,msize,lastend,mmap->type);
        if (addr > lastend && lastend<__max_phy_mem_addr) {
            ulong hsize = addr - lastend;
            ulong haddr;

            printk("hole:%lx length:%lx\n", lastend, addr - lastend);

            if (hsize < PAGE_SIZE_LARGE)
                deal_reserve(lastend, addr - lastend, false);
            else {
                if (lastend & (PAGE_SIZE_LARGE - 1)) {
                    deal_reserve(lastend,
                                 PAGE_SIZE_LARGE - (lastend & (PAGE_SIZE_LARGE - 1)),
                                 false);
                    lastend += PAGE_SIZE_LARGE;
                    lastend &= ~(PAGE_SIZE_LARGE - 1);
                }
                for (haddr = lastend; haddr + PAGE_SIZE_LARGE < addr;
                     haddr += PAGE_SIZE_LARGE) {
                    if (OK !=
    ...
    remap_iomem();
    extern char  ro_start[],  ro_end[];
    char *vaddr = ro_start;
    while(vaddr<ro_end) {
        printk("vaddr:%lx,paddr:%lx\n",K2P(vaddr), vaddr);
        map_4k_page_p2v(K2P(vaddr), (ulong) vaddr, map_flags_readonly()); 
        map_4k_page_p2v(K2P(vaddr), (ulong) P2V(K2P(vaddr)), map_flags_readonly());

        vaddr = vaddr + PAGE_SIZE_SMALL;
...
//print_regs();
    ...
    if (magic != MULTIBOOT_BOOTLOADER_MAGIC) {
        printk("bootload magic do not match %x,%x,addr:%x\n", magic,
               MULTIBOOT_BOOTLOADER_MAGIC,info_addr);
        cli_hlt();
    }
    remaped = 0;
    e820map_addr = 0;
    ...
    if (CHECK_FLAG(mbi->flags, 6)) {
        ...
        e820map_addr = (ulong) P2K(mbi->mmap_addr);
...
//print_regs();
        ...
        dump_mem((char *)addr,0x100);
        dump_mem((char *)0x9000,0x100);
