...
static void msec_callback(u64 nsec,void *param)
{
   ...
   //msec_count++
   set_timeout_nsec_cb(1000000, msec_callback,param);
   run_local_timers();
}
static void msec_apcallback(u64 nsec,void *param)
{
   extern u64 msec_count;
   void run_local_timers(void);
   set_timeout_nsec_cb(1000000, msec_apcallback,param);
   run_local_timers();
}

static __init int init_irq_call(bool isbp)
{
    if(isbp)set_timeout_nsec_cb(1000000, msec_callback,current);
    else set_timeout_nsec_cb(1000000, msec_apcallback,current);
