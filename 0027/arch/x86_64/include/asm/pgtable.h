...
#define IO_MEM_BASE	0xffffffe800000000UL
#define KERNEL_BASE	0xffffffff80000000UL
#define __PHYS_VIRT_START  0xFFFF820000000000UL
...
//below kernel start 1G
#define KERNEL_BELOW_1G_START	(KERNEL_BASE- 0x40000000UL)
#define KERNEL_BELOW_1G_END	KERNEL_BASE
#define KERNEL_BELOW_1G_SIZE  	0x40000000UL
...
extern void unmap_free_at_impl(ulong vaddr, size_t size,bool isfree);
static inline void unmap_at(ulong addr, size_t size)
{
    unmap_free_at_impl(addr, size, false);
}
static inline void unmap_free_at(ulong addr, size_t size)
{
    unmap_free_at_impl(addr, size, true);
}
...
static inline ulong map_flags_readonly()
{
    return PTE_P|PTE_PS;
}
