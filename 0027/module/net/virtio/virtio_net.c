...
struct record_tx;
struct record_tx{
    struct record_tx *pnext;
    uintptr_t mask;
    uintptr_t addr;
    size_t size;
    char   buf[0];
};
struct record_tx *phead_tx=NULL;
void add_record(void *addr,size_t size)
{
    struct record_tx *p = yaos_malloc(size+sizeof(struct record_tx));
    p->pnext = phead_tx;
    p->mask = ~(uintptr_t)addr;
    p->addr = (uintptr_t)addr;
    phead_tx = p;
    p->size = size;
    memcpy(&p->buf[0], addr, size);
}
void dump_record()
{
   struct record_tx *p = phead_tx;
   while(p) {
      printk("p:%lx,next:%lx,mask:%lx,addr:%lx,size:%d\n",p,p->pnext,p->mask,p->addr,p->size);
      if ((uintptr_t)p->addr != ~p->mask) {
        printk("*******************************************\n");
        dump_mem((char *)((uintptr_t)p - 0x1000),0x1000);
      }
      dump_mem(p->buf,p->size);
      p=p->pnext;
   }
}
void check_record()
{
   ...
   while(p) {
      if ((uintptr_t)p->addr != ~p->mask) {
        printk("*******************************************check record fail:%lx,%lx,%d\n",p,p->addr,p->size);
        dump_mem((char *)((uintptr_t)p - 0x1000),0x2000);
      }
      p=p->pnext;
   }
...
static int poll_callback(struct poll_cb *p)
{
    ...
    while (!STAILQ_EMPTY(&poll_list)) {
        ...
        if (pci_vn.receiver) {
           //add_record(pkt->vdata + pci_vn._hdr_size, pkt->len-pci_vn._hdr_size);
...
int virtio_net_send_packets(struct iovec * vecs, int nr, void *data)
{
    ...
    for(int i=1;i<=nr;i++) {
        ...
        //add_record(vecs[i-1].iov_base, vecs[i-1].iov_len);
        if (showTcp) {
    ...
    int ret = 0;
    WITH_SOFTIRQ_LOCK() {
        ret = virtio_to_queue(to_virtio_dev_t(&pci_vn), TX_Q, phys, nr+1, data);
    }
    if (ret) printk("virtio_to_queue error:%d\n",ret); 
    return ret;

}
int virtio_net_send_packet(void *buf,size_t size, void *data)
{
    ...
    int ret = 0;
    WITH_SOFTIRQ_LOCK() {
        ret = virtio_to_queue(to_virtio_dev_t(&pci_vn), TX_Q, phys, 2, data);
    }
    return ret;
...
static void rx_queue_callback(void *data, size_t len)
{
...
{
    virtio_read_queue(to_virtio_dev_t(&pci_vn), RX_Q);
...
static void tx_queue_callback(void *data, size_t len)
{
    if (pci_vn.tx_done)(*pci_vn.tx_done)(data);
    virtio_net_stats.ets_packetT++;
}
static int tx_poll_callback(struct poll_cb *poll)
{
    virtio_read_queue(to_virtio_dev_t(&pci_vn), TX_Q);
    return 0;
#if 0
    void  *p;
    size_t len;
    while (virtio_from_queue(to_virtio_dev_t(&pci_vn), TX_Q, (void **)&p, &len)
           == 0) {
        if(pci_vn.tx_done) (*pci_vn.tx_done)(p);
...
void virtio_net_dump_queue()
{
    extern void virtio_dump_queue(virtio_dev_t, int q);
    dump_record();
    virtio_dump_queue(to_virtio_dev_t(&pci_vn), TX_Q);
}
int init_virtio_net(int (*setup)(struct virtio_net *dev),void *param)
{
    ...
    if (virtio_setup_device(dev, VIRTIO_NET_SUBID, 16)) {
        ...
        virtio_set_queue_callback(dev, RX_Q, rx_queue_callback);
        virtio_set_queue_callback(dev, TX_Q, tx_queue_callback);
