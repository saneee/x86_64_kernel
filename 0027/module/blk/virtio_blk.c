...
static void blk_queue_callback(void *data, size_t len)
{
    struct blk_req *p = (struct blk_req *)data;

    wake_up_thread(p->thread);
}
static void virtio_blk_check_queue(void)
{
    virtio_read_queue(to_virtio_dev_t(&pci_vblk),0);

}
__used static void vblk_timeout(u64 nowmsec,void *param)
{
    set_timeout_nsec_poll(5000000000, vblk_timeout,param);
...
static inline void virtio_blk_get_driver_features(virtio_dev_t dev)
{ 
...
int init_virtio_blk()
{
    ...
    if (virtio_setup_device(dev, VIRTIO_BLK_DEVICE_ID, 1)) {
        ...
        virtio_set_queue_callback(dev, 0, blk_queue_callback);
