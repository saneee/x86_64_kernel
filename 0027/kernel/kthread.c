...
void thread_add_poll(void *p,int lvl)
{
    ...
    ASSERT_PRINT((ulong)p<(ulong)ro_start || (ulong)p>(ulong)ro_end, "p=%lx\n",p);
    ASSERT_PRINT((ulong)pcb->thread<(ulong)ro_start || (ulong)pcb->thread>(ulong)ro_end, "thread=%lx\n",pcb->thread);
