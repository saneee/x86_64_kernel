...
static ret_t s_alloc_linear(vm_zone_t id,void *addr, size_t size)
{
    ret_t ret;
    ret.v = (ulong) alloc_linear_at(id, addr, size);
    ret.e = ret.v == 0 ? ERR_YAOSCALL_NO_MEM : YAOSCALL_OK;
    return ret;
...
{
    ret_t ret = {0,0};
    if (addr & (MIN_VM_ALLOC_SIZE-1)) return ret;
...
static ret_t s_unmap_at(ulong addr, size_t size)
{
    //printk("unmap_at：%lx,size:%lx\n",addr,size);
    unmap_at(addr, size);
    ret_t ret={0,0};
    return ret;
}
...
static ulong s_map_flags_at(ulong addr)
{
    ret_t ret = map_paddr_flags_at(addr);
    return ret.e;
}
...
DECLARE_YAOSCALL(YAOS_map_flags_at, s_map_flags_at);
DECLARE_YAOSCALL(YAOS_unmap_at, s_unmap_at);
#undef weak_alias
